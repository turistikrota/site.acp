...
const CategoryEditView = () => {
  ...
  const { data, isLoading: isDetailLoading } = useQuery(
    apiUrl(Services.Category, `/admin/${params.uuid}`)
  );
  ...
  const [trMarkdown, setTrMarkdown] = useMdContent(
    Config.cdn.buildUrl(Config.cdn.apps.categoriesMd, `${params.uuid}.tr.md`)
  );
  const [enMarkdown, setEnMarkdown] = useMdContent(
    Config.cdn.buildUrl(Config.cdn.apps.categoriesMd, `${params.uuid}.en.md`)
  );
  ...
  const getParentCategoryDetails = async (parentIds) => {
    if (parentIds.length === 0) return;
    setCategoryLoading(true);
    const res = await httpClient.get(
      apiUrl(
        Services.Category,
        `/admin/parents?mainUUIDs=${parentIds.join(",")}`
      )
    );
    ...
    if (res.status === 200 && res.data) {
      form.setFieldValue(
        ...
        res.data.list.map((p) => ({
          uuid: p.uuid,
          name: p.meta[i18n.language].name,
        }))
      );
    }
  };
  ...
  return (
    <ClaimGuardLayout
      ...
      roles={[Roles.admin, Roles.Categories.any, Roles.Categories.update]}
    >
      <PageContentLayout>
        <RBreadcrumb title={t("edit.title")}>
          ...
          <RBreadcrumb.Current>{t("edit.title")}</RBreadcrumb.Current>
        </RBreadcrumb>
...
export { CategoryEditView as Component };
